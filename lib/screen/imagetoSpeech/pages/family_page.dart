...
class _FamilyPageState extends State<FamilyPage>
  ...
  final String _category = 'Family';
  final Color _categoryColor = Colors.deepPurple;
  ...
  void _initTts() {
    // GoogleTtsService uses VoiceSettingsService for configuration
  }

  Future<void> _loadImages() async {
    setState(() => _isLoading = true);
    try {
      final images = await _imageService.getImagesByCategory(_category);
      setState(() {
        _images = images;
        _isLoading = false;
      });
  ...
  Future<void> _speak(String text) async {
    try {
      ...
      await _flutterTts.speak(text: text, languageCode: 'en-US');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Icon(Icons.family_restroom, color: _categoryColor),
            const SizedBox(width: 12),
            Text(
              _category,
              style: TextStyle(
